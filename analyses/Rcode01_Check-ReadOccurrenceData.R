...
vert.snap <- readxl::read_excel(here::here("data/raw-data/SNAP-Vertebrate-Species_ActT-Diet-ForagS-NestH-Morpho-HabPref-DispD-MoveMod-PressureTraits.xlsx"))
...
group_by(lst.files.df, file.nme) %>% do(clean_occurGBIF(ffile = .$file.nme, year.min = 2010, uncertainty = 1000) %>% get_griddedOccur(sp.occur = ., year.todo = c(2010:2020), nlim = 5))
...
######################################################
# Check INPN data to see if we can add some species
######################################################
# Load data summary 
...
vert.snap.trait <- vert.snap[!is.na(vert.snap$dispersal_km),] #species for which we have trait data 
...
openxlsx::write.xlsx(vert.snap.final, here::here('data/derived-data/FinalList-of-SNAP-Vertebrate-Species_ActT-Diet-ForagS-NestH-Morpho-HabPref-DispDTraits.xlsx'))
openxlsx::write.xlsx(missing, here::here('data/derived-data/DataDeficient-SNAP-Vertebrate-Species.xlsx'))

missing.trait <- missing[missing$LB_NOM_VALIDE_SPE_LEVEL_SYN %in% final$SpeciesName,]
openxlsx::write.xlsx(missing.trait, here::here('data/derived-data/DataDeficientTraits-SNAP-Vertebrate-Species.xlsx'))
missing.occur <- missing[missing$LB_NOM_VALIDE_SPE_LEVEL_SYN %in% vert.snap.trait$LB_NOM_VALIDE_SPE_LEVEL_SYN,]
openxlsx::write.xlsx(missing.occur, here::here('data/derived-data/DataDeficientOccurrences-SNAP-Vertebrate-Species.xlsx'))
...
openxlsx::write.xlsx(missing.both, here::here('data/derived-data/DataDeficientTraits&Occurrences-SNAP-Vertebrate-Species.xlsx'))

missing.mmode <- vert.snap.final[is.na(vert.snap.final$mov.mode.climber),]
openxlsx::write.xlsx(missing.mmode, here::here('data/derived-data/DataDeficientMoveMode-SNAP-Vertebrate-Species.xlsx'))
...
########## Check IUCN range ##########################
######################################################
missing.occur <- openxlsx::read.xlsx(here::here('data/derived-data/DataDeficientOccurrences-SNAP-Vertebrate-Species.xlsx'))
...
for (l in lst.files) {
  ...
  if (sum(rr.poly$Nocc!=0) >= nlim) {
    ...
    sf::st_write(rr.poly, here::here(paste0('data/derived-data/SIG/Occurrence/AllYearsCombined/IUCNOccurrenceData_France_',
                                         gsub(' ','_', lb.sp),'_Res1000m', '.gpkg')), driver = 'GPKG', delete_layer = T)
    ...
    terra::writeRaster(rr, here::here(paste0('data/derived-data/SIG/Occurrence/AllYearsCombined/IUCNOccurrenceData_France_',
                                                    gsub(' ','_', lb.sp),'_Res1000m_', '.tif')), overwrite = T)
